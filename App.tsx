  ...
  return (
    <div className={`min-h-screen min-h-[100dvh] w-full overflow-y-auto overflow-x-hidden flex flex-col transition-colors duration-500 hide-scrollbar ${theme === 'classic' ? 'text-black bg-[#F5F5F5]' : 'text-white bg-[#0a0a0a]'}`}>
      ...
      <header className="h-12 sm:h-14 shrink-0 sticky top-0 z-20 flex items-center justify-center px-4 backdrop-blur-sm bg-inherit">
        <span className={`font-bold text-lg sm:text-xl tracking-tighter text-center ${theme === 'classic' ? 'text-wnrs-red' : 'text-white'}`}>
          THE PRAWN GAME
        </span>
        ...
        <div className="absolute right-4 flex items-center gap-1 sm:gap-2">
          ...
          <Button variant="icon" onClick={() => setTheme(prev => prev === 'classic' ? 'midnight' : 'classic')}>
            {theme === 'classic' ? <Moon size={18} className="sm:w-5 sm:h-5" /> : <Sun size={18} className="sm:w-5 sm:h-5" />}
          </Button>
        </div>
      </header>
            ...
            >
              <div className="flex-1 flex flex-col items-center justify-center w-full">
                <div className="text-center mb-4 sm:mb-6">
                  <h2 className="text-xl sm:text-2xl font-bold mb-1">Card Options</h2>
                  <p className="opacity-60 text-xs sm:text-sm">Choose which card sets to include.</p>
                </div>
                  ...
                  {cardSets.map((cardSet) => (
                    ...
                    >
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <div>
                          <span className="font-medium block text-sm">{cardSet.name}</span>
                        </div>
                      </div>
              ...
              <div className="w-full mt-4 sm:mt-6 pb-safe">
                <Button fullWidth onClick={startGame} disabled={!hasEnabledCardSet}>
